...
func TestNewLinajeParser(t *testing.T) {
	tests := []struct {
		name string
		want *LinajeParser
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := newLinajeParser(); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewLinajeParser() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestLinajeParser_Parse(t *testing.T) {
	type args struct {
		file string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("LinajeParser.Parse() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("LinajeParser.Parse() = %v, want %v", got, tt.want)
			}
		})
	}
}
